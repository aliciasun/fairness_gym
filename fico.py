...
def read_totals(data_dir):
    ...
    frame = cleanup_frame(pd.read_csv(
        os.path.join(data_dir, FILES['overview']),index_col=False))
...
def parse_data(data_dir, filenames=None):
    ...
    cdfs = cleanup_frame(pd.read_csv(
        os.path.join(data_dir, filenames[0]),index_col='Score'))
    performance = 100 - cleanup_frame(pd.read_csv(
        os.path.join(data_dir, filenames[1]),index_col='Score'))
...
def get_data_args(data_dir='data/loan/'):
...
def get_marginal_loan_repaid_probs(data_dir='data/loan/'):
    ...
    loan_repaid_probs = [
        loan_repaid_probs, loan_repaid_probs
